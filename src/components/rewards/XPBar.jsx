...
export function XPBar({ totalXP = 0, showTitle = false }) {
  const { level, title, emoji, current, needed, pct, isMax } = xpToNextLevel(totalXP);
  return (
    <div
      className="flex items-center gap-2 bg-white border-[3px] border-ink/15 rounded-pill shadow-chunky px-3 py-1.5"
    ...
    >
      ...
      <div className="flex-1 min-w-[100px]">
        {showTitle && (
          <div className="font-display font-extrabold text-xs leading-none mb-1 truncate">
            {emoji} {title}
          </div>
        )}
